  ...
  function getFileDescription(fileItem) {
      ...
      const city = getPropertyValue('P131')
            ? vm.monument.valuesLabels[getPropertyValue('P131').id].en ||
              ...
              vm.monument.valuesLabels[getPropertyValue('P131').id][Object.keys(vm.monument.labels)[0]]
            : undefined;
      const date = fileItem.metadata.DateTimeOriginal ? parseEXIFTime(fileItem.metadata.DateTimeOriginal) : undefined;
      const monumentTemplate = vm.country
            ? vm.country.template || `{{${vm.country.templateName}|${vm.country.monumentId}}}`
            : `{{on Wikidata|${vm.monument.id}}}`;

      return `=={{int:filedesc}}==
{{Information
|description={{en|1=${city ? `${city}, ` : ''}${name.value}.}}${monumentTemplate}
...
=={{int:license-header}}==
{{self|cc-by-sa-4.0}}
${vm.country && vm.country.code === 'pl' ? '{{Wikiwakacje 2017|monuments}}' : ''}${vm.country
            ? `{{subst:WLM-is-running|${vm.country.code}|{{Wiki Loves Monuments 2017|${vm.country.code}}}}}`
            : ''}
...
[[Category:Uploaded with Monumental]]`;
  ...
  function getCategory() {
      ...
      if (vm.country.code === 'ie') {
          return '[[Category:Images from Wiki Loves Monuments 2017 in Ireland – missing category]]';
        }
  ...
  function readEXIF(fileItem) {
      EXIF.getData(fileItem._file, function file() {
          ...
          if (fileItem.metadata.thumbnail && fileItem.metadata.thumbnail.blob) {
              fileItem.thumbnail = URL.createObjectURL(fileItem.metadata.thumbnail.blob);
            }
          ...
          const city = getPropertyValue('P131')
                ? vm.monument.valuesLabels[getPropertyValue('P131').id][vm.lang.code] ||
                  ...
                  vm.monument.valuesLabels[getPropertyValue('P131').id][Object.keys(vm.monument.labels)[0]]
                : undefined;
          let time = fileItem.file.lastModifiedDate ? fileItem.file.lastModifiedDate.toISOString().replace(/([-TZ:]|\.[0-9]*)/g, '') : '';
          if (fileItem.metadata.DateTimeOriginal) {
              const parsed = parseEXIFTime(fileItem.metadata.DateTimeOriginal);
              time = `${parsed.date}${parsed.time || ''}`.replace(/([-TZ:]|\.[0-9]*)/g, '');
            }

          const ext = fileItem.file.name.split('.').pop().toLowerCase();
          fileItem.commons.fileName = `${city ? `${city} - ` : ''}${name.value} - ${time}.${ext}`;
          fileItem.commons.description = getFileDescription(fileItem);

          fileItem.formData.push(
                ...[{ filename: fileItem.commons.fileName }, { text: fileItem.commons.description }],
            );
  ...
  function getValueLabel(vid) {
      ...
      return labels[vid][vm.lang] || labels[vid].en || labels[vid][Object.keys(labels[vid])[0]];
    }
  ...
  function init() {
      ...
      wikidata.getById(id).then((data) => {
          ...
          $window.document.title = `${title ? title.value : vm.monument.id} – Wiki Loves Monuments Map`;
        });
    }
